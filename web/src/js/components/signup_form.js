...
class SigninForm extends LitElement {
  ...
  async _handleSubmit(event) {
    ...
    try {
      const response = await fetch("/v1/user/create", {
        method: "POST",
        headers: {
  ...
  render() {
    return html`
      <form @submit="${this._handleSubmit}">
        <div class="greetings">
          <p class="greetings-header">Welcome</p>
          <p class="greetings-comment">Create an account to get started.</p>
        </div>
        <div>
        ...
        <div class="input-group">
          ...
          <button
            type="button"
            class="toggle-password"
            @click="${this._toggleConfirmPassword}"
          >
            ${this.showConfirmPassword ? "Hide" : "Show"}
          </button>
        </div>
        <button type="submit">Submit</button>
        <p class="error" ?hidden="${!this.passwordsDontMatch}">
...
customElements.define("signup-form", SigninForm);
