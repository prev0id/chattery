...
func TestService_firstPageOfDMMessages(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
	...
	}{
		{
			...
			prepare: func(f *serviceFields, a *args) {
				...
				f.db.EXPECT().
					FirstPageOfDMMessages(a.ctx, a.cursor).
					Return([]*domain.DMMessage{firstMessage, secondMessage}, nil)
				f.db.EXPECT().
					SetDMLastReadMessage(a.ctx, a.cursor.ChatID, a.userID, firstMessage.ID).
					Return(nil)
			},
			args: args{
				ctx: ctx,
		...
		},
		{
			...
			expected: expected{
				err:   dependencyError("s.db.FirstPageOfDMMessages"),
				limit: limit,
			},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			messages, nextCursor, err := fields.service.firstPageOfDMMessages(
				tt.args.ctx,
				tt.args.userID,
				tt.args.cursor,
			)
